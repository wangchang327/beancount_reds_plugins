...
default_rules = {
    "cash_and_fees": (  # Open cash and fees accounts
        ...
        [
            ...
            (
                "Expenses:Fees-and-Charges:Brokerage-Fees:{taxability}:{account_name}",
                "{f_opcurr}",
            ),
        ],
    ),
    "commodity_leaves_income": (  # Open common set of investment accounts with commodity leaves
        ...
        [
            (
                "Income:{subroot}:{taxability}:Dividends:{account_name}:{f_ticker}",
                "{f_opcurr}",
            ),
            (
                "Income:{subroot}:{taxability}:Interest:{account_name}:{f_ticker}",
                "{f_opcurr}",
            ),
            (
                "Income:{subroot}:{taxability}:Capital-Gains:{account_name}:{f_ticker}",
                "{f_opcurr}",
            ),
        ],
    ),
    ...
    (
        ...
        [
            (
                "Income:{subroot}:{taxability}:Capital-Gains-Distributions:Long:{account_name}:{f_ticker}",
                "{f_opcurr}",
            ),
            (
                "Income:{subroot}:{taxability}:Capital-Gains-Distributions:Short:{account_name}:{f_ticker}",
                "{f_opcurr}",
            ),
        ],
    ),
...
def run_rule(rules, rulename, f_acct, f_ticker, f_opcurr, entry):
    try:
        ...
        print(
            f"WARNING (opengroup): {rulename} not found in rules. "
        ...
        for m in entry.meta:
            if m.startswith("opengroup_") or m.startswith("closegroup_"):
                ...
                for leaf in entry.meta[m].split(","):
                    for acc_params in run_rule(
                        rules, rule, entry.account, leaf, op_currency, entry
                    ):
                        ...
                        if oc == "opengroup":
                            ...
                            new_entries.append(
                                data.Close(meta, entry.date, acc_params[0])
                            )
